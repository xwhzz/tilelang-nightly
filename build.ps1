...
$apt_command="apt update && apt install -y software-properties-common && add-apt-repository -y ppa:deadsnakes/ppa && apt update && apt install -y wget curl libtinfo-dev zlib1g-dev libssl-dev build-essential libedit-dev libxml2-dev && echo `$PATH"
...
$uid = $currentUser.User.Value 
$gid = $currentUser.Groups[0].Value
...
docker run --rm -v "${PWD}:/tilelang" `
  ...
  -e HOST_GID=$gid `
  $IMAGE /bin/bash -c `
  "$apt_command && $install_python_env && $install_cmake && $install_pip && $run_command"